...
/* ─── Detail Modal ────────────────────────────────────────────────── */
const DetailModal = ({ tech, onClose, onUpdateAccess }) => {
  const [loading, setLoading] = useState(false);
  const access = tech.loginAccess;

  const handleUpdate = async (newAccess) => {
    setLoading(true);
    await onUpdateAccess(tech.id, newAccess);
    setLoading(false);
  };

  const cfg = ACCESS_CONFIG[access] || ACCESS_CONFIG.PENDING;
    ...
    >
      ...
      >
        ...
        <div className="px-6 mb-4">
          <span
            className={`inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-full border ${cfg.badge}`}
          >
          ...
          {access === 'PENDING' && (
            <>
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-bold text-sm bg-red-500 hover:bg-red-600 text-white transition-all disabled:opacity-60"
              >
                {loading ? (
                  <span className="material-symbols-outlined text-[18px] animate-spin">progress_activity</span>
                ) : (
      ...
      <div className="space-y-5 sm:space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-400 mt-1">
              Manage all registered technicians on the platform.
            </p>
          </div>
        ...
        {!isLoading && !error && technicians.length === 0 && (
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm py-14 text-center">
            ...
            <p className="text-slate-500 font-semibold mt-3">No technicians registered yet.</p>
          </div>
        )}
        ...
        {!isLoading && filtered.length > 0 && (
          <div className="bg-white rounded-2xl border border-slate-100/80 shadow-sm overflow-hidden">
            <div className="divide-y divide-slate-50">
              {filtered.map((tech) => {
                const cfg = ACCESS_CONFIG[tech.loginAccess] || ACCESS_CONFIG.PENDING;
                return (
                  <div
                    key={tech.id}
                    ...
                    {tech.profilePhotoUrl ? (
                      ...
                      <div
                        className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center text-white font-bold text-base sm:text-lg shrink-0
                          ${tech.loginAccess === 'BLOCK' ? 'bg-slate-300' : 'bg-gradient-to-br from-[#004ac6] to-[#57dffe]'}`}
                      >
                        {tech.name?.charAt(0).toUpperCase()}
                      </div>
                    )}
