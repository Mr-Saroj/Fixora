...
const PricingCard = ({ title, price, desc, features, popular, contact }) => (
  <div className={`p-5 sm:p-8 rounded-3xl flex flex-col justify-between h-full relative transition-all duration-500 reveal-card ${
    popular 
    ...
    <div>
      {/* Reduced bottom margin on mobile (mb-4) */}
      <div className="mb-4 sm:mb-6">
        {/* Slightly smaller title on mobile */}
        <h3 className="text-[20px] sm:text-[24px] font-bold text-text-main">{title}</h3>
        {desc && <p className="text-[12px] sm:text-[13px] text-text-muted mt-1">{desc}</p>}
        
        {/* Reduced top margin (mt-2) and text size for price on mobile */}
        <div className="mt-2 sm:mt-4 flex items-baseline">
          <span className="text-[28px] sm:text-[36px] font-extrabold text-text-main tracking-tight">${price}</span>
          <span className="text-text-muted ml-1 text-[13px] sm:text-[15px] font-medium">/mo</span>
        </div>
      </div>

      {/* Reduced bottom margin on divider (mb-4) */}
      <div className="w-full h-[1px] bg-black/[0.06] mb-4 sm:mb-6"></div>

      {/* Reduced space between list items (space-y-2) and bottom margin (mb-5) on mobile */}
      <ul className="space-y-2 sm:space-y-3.5 mb-5 sm:mb-8 text-[13px] sm:text-[14px]">
        {features.map((f, i) => (
          <li key={i} className="flex items-center text-text-muted">
            <span className="material-symbols-outlined text-primary mr-2 sm:mr-3 text-[18px] sm:text-[20px] shrink-0">check</span> 
            <span>{f}</span>
          </li>
        ))}
      </ul>
    ...
    {/* Reduced button padding (py-2.5) and text size on mobile */}
    <button className={`w-full py-2.5 sm:py-3.5 rounded-xl font-medium text-[14px] sm:text-[15px] transition-all duration-300 active:scale-95 ${
      popular 
