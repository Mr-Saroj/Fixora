...
// 🔑 Replace with your own LocationIQ access token (free, no credit card required)
// Get one at: https://my.locationiq.com/register
const LOCATIONIQ_ACCESS_TOKEN = 'YOUR_LOCATIONIQ_ACCESS_TOKEN';
...
const Request = () => {
  ...
  // Fetch current location using Geolocation API + LocationIQ reverse geocoding
  const fetchCurrentLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        // LocationIQ reverse geocoding — Nominatim-compatible, no card required, 5,000 req/day free
        // zoom=18 = building/street level detail (most granular)
        const url = `https://us1.locationiq.com/v1/reverse?key=${LOCATIONIQ_ACCESS_TOKEN}&lat=${latitude}&lon=${longitude}&format=json&addressdetails=1&namedetails=1&zoom=18`;

        fetch(url)
          .then((res) => {
            if (!res.ok) throw new Error('LocationIQ request failed');
            return res.json();
          })
          .then((data) => {
            const addr = data?.address || {};

            // Prefer the most local/specific name — village, hamlet, and
            // suburb/neighbourhood are often missing from display_name
            // even though they're present in the structured address block.
            const localArea =
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log('Form Submitted:', { ...formData, photos: uploadedPhotos, coordinates });
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl mx-auto space-y-6 pb-12">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-100 rounded-xl pl-10 pr-4 py-2.5 text-sm text-slate-700 placeholder-slate-400 focus:ring-2 focus:ring-[#004ac6]/20 focus:border-[#004ac6] outline-none transition-all"
                />
              </div>
            </div>
          ...
          <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <select
                    ...
                    onChange={handleInputChange}
                    required
                    className="w-full bg-slate-50 border border-slate-100 rounded-xl pl-10 pr-4 py-2.5 text-sm text-slate-700 focus:ring-2 focus:ring-[#004ac6]/20 focus:border-[#004ac6] outline-none transition-all appearance-none"
                  >
              ...
              <div>
                ...
                <div className="flex gap-2 h-[42px]">
                  <button
                    ...
                    className={`flex-1 rounded-xl text-xs font-bold transition-all duration-200 border flex items-center justify-center gap-1.5
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-sm text-slate-700 placeholder-slate-400 focus:ring-2 focus:ring-[#004ac6]/20 focus:border-[#004ac6] outline-none transition-all resize-none leading-relaxed"
              />
        ...
        <div className="bg-white rounded-2xl border border-slate-100/80 shadow-[0_4px_6px_-1px_rgba(0,0,0,0.02),0_10px_30px_-8px_rgba(0,0,0,0.04)] overflow-hidden">
          ...
          <div className="p-6 space-y-4">
            ...
            <button
              ...
              className={`w-full flex items-center justify-center gap-2.5 py-3 rounded-xl border text-sm font-semibold transition-all duration-200
        ...
        <div className="bg-white rounded-2xl border border-slate-100/80 shadow-[0_4px_6px_-1px_rgba(0,0,0,0.02),0_10px_30px_-8px_rgba(0,0,0,0.04)] overflow-hidden">
          ...
          <div className="p-6">
            ...
            <div
              ...
              className="border-2 border-dashed border-slate-200 rounded-2xl p-8 text-center cursor-pointer hover:border-[#004ac6]/40 hover:bg-[#004ac6]/[0.02] transition-all duration-200 group"
            >
            ...
            {uploadedPhotos.length > 0 && (
              <div className="mt-5 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                {uploadedPhotos.map((photo, index) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="absolute top-1.5 right-1.5 w-6 h-6 bg-red-500 text-white rounded-full flex items-center justify-center opacity-0 group-hover/photo:opacity-100 transition-all duration-200 hover:bg-red-600 shadow-sm"
                    >
                ...
                {uploadedPhotos.length < 5 && (
                  <div
                    onClick={() => fileInputRef.current?.click()}
                    className="aspect-square rounded-xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-center cursor-pointer hover:border-[#004ac6]/40 hover:bg-[#004ac6]/[0.02] transition-all duration-200"
                  >
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-end">
          <button
            type="button"
            onClick={handleReset}
            className="px-6 py-3 rounded-xl border border-slate-200 text-slate-600 font-semibold text-sm hover:bg-slate-50 hover:border-slate-300 transition-all duration-200 flex items-center justify-center gap-2"
          >
